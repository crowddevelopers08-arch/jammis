...
interface ConsultationFormPopupProps {
  ...
  pageSource?: string // Optional prop to specify source manually
}
...
const ConsultationFormPopup: React.FC<ConsultationFormPopupProps> = ({
  ...
  // Capture the current URL when component mounts or when popup opens
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      // Log for debugging
      console.log("Current URL captured:", url)
    }
  }, [open]) // Re-capture when popup opens/closes
  ...
  const getSourceDetails = () => {
    if (typeof window === "undefined") return { url: "", pageTitle: "", referrer: "" }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Create a comprehensive source string with all available context
      const sourceString = pageSource || sourceDetails.url
      
      // Also store additional context in a separate field for better tracking
      const contextData = {
        fullUrl: sourceDetails.url,
        pageTitle: sourceDetails.pageTitle,
        referrer: sourceDetails.referrer,
        pathname: sourceDetails.pathname,
        queryParams: sourceDetails.queryParams,
      ...
      const leadData = {
        ...
        condition: formData.concerns, // Changed from 'concerns' to 'condition' to match schema
        message: formData.hairProblems,
        pincode: formData.pincode,
        consent: true,
        source: sourceString, // This will store the live URL
        formName: "Hair Consultation Form",
        // Include additional context in message or a separate field if needed
        // You could also add a new field to your schema for this
        additionalContext: JSON.stringify(contextData),
      }

      console.log("Submitting lead with source:", leadData.source)
